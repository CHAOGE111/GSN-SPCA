DMESPCA = function(X, k=2, overlap.group, k.group=2, we=0.5, t = 0.1, niter= 20, err=0.0001, Num.init=5){
  # --------------------------------------------------------------------------
  ...
  tX = X
  out = rank1.DMESPCA(tX, overlap.group, k.group, we, t, niter, err, Num.init)
...
# ----------------------------------------------------------------------------
rank1.DMESPCA = function(X, overlap.group, k.group, we, t, niter=20, err=0.0001, Num.init = 5){
  n = nrow(X); # n is the number of samples
  p = ncol(X); # p is the number of features
  d.opt = -100
  # set five initial points
  for(ii in 1:Num.init){
    we1 = we
    print(paste("rank iteration:", ii))
    set.seed(ii*100)
    ...
    for (i in 1:niter){
      ...
      # print(paste("Iteration:", i, "Error U:", norm(u - u0, 'E'), "Error V:", norm(v - v0, 'E')))
    ...
    # Print final results of each rank iteration
    # print(paste("Final d:", d.opt, "at iteration:", ii))
  }
  return (list(u=u.opt, v=v.opt, d=d.opt))
}

# ----------------------------------------------
cycleFun2 = function(X, UU, overlap.group, k.group,we, t, niter=20, err=0.0001, Num.init = 5){
  n = nrow(X); # n is the number of samples
  p = ncol(X); # p is the number of features
  d.opt = -100
  for(ii in 1:Num.init){
...
apply_weights = function(data, weights) {
  if (nrow(data) != nrow(weights) || ncol(data) != ncol(weights)) {
    stop("Dimensions of data and weights do not match.")
  }
  weighted_data = data * weights
  return(weighted_data)
}

# Modified overlap.group.penalty function
