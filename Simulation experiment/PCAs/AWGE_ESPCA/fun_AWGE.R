AWGE = function(X, k=2, overlap.group, k.group=2, we=0.5, t = 0.1, niter= 20, err=0.0001, Num.init=5){
    n = nrow(X); # n is the number of samples
    p = ncol(X); # p is the number of features
    U = matrix(0,n,k); D = matrix(0,k,k); V = matrix(0,p,k)
    tX = X
    out = rank1.AWGE(tX, overlap.group, k.group, we, t, niter, err, Num.init)
...
rank1.AWGE = function(X, overlap.group, k.group, we, t, niter=20, err=0.0001, Num.init = 5){
    n = nrow(X); # n is the number of samples
    p = ncol(X); # p is the number of features
    d.opt = -100
...
boxplot_outlier_correction = function(X, LQ = 0.10){
